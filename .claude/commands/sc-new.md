...
| *(確認待ち)* | ⚠️ナレーション要確認がある場合のみ停止（サムネイル・BGMは自動確定） | 通常0分 |
...
### サムネイル・BGMの自動確定（2026-07-21〜）

サムネイル・BGMは**ユーザーの手動確認を待たず自動的に確定する**。

**サムネイル:**
- 生成後、Claude が Read ツールで画像を直接見て判断する（文字の可読性・時代考証・破綻の有無）。
- 問題なければそのまま採用。明らかな問題（文字化け・崩れた構図等）があれば1回だけ自動再生成し、それでも改善しなければ注記した上でそのまま採用する（ユーザーには止めずに進む）。

**BGM:**
- 役割別6曲（各役割2候補）を用意した後、`/consult` スキルを使い、エピソードのトーン・
  3曲構成の制約（役割ごとに固定音量ループ、曲間はクロスフェード4秒、序盤→中盤→終盤で
  緊張→高揚→余韻の流れを作る）を踏まえて**各役割1曲ずつ計3曲**に自動的に絞り込む。
- `/consult` の最終提案に従って採用し、STEP 4 のBGM紐付け処理へそのまま進む。
- 選ばれなかった3曲・使用しなかった credit.txt は通常通り削除する。

### 完了報告

制作確認書のファクトチェック・QA結果のみを画面に表示する。サムネイル・BGMは上記の通り自動確定済みなので報告に含めない。
...
❌自動修正済み・⚠️なし → 「✅ QA完了 — 問題なし」のみ表示して即 STEP 4 へ自動的に進む（確認を待たない）。
⚠️がある場合のみ内容を表示してユーザーに確認を取る（この場合のみ止まる）。
...
- **サムネイル・BGMを見直したい**（自動確定後でもユーザーが望めば）→ 該当ファイルを削除して再生成・再選定する。
- **すべてOK** → STEP 4 へ進む
