...
- 差し替え希望があれば該当役割のみ再選定（別候補への切替、または新規クエリで再検索）し、
  再度試聴確認を求める。
- OK後、選ばれなかった曲・使用しなかった credit.txt を削除する。
...
### ユーザーによる目視確認（必須・2026-07-28〜）

自動QA（WARNING対応含む）が完了したら、`images/` フォルダの全シーン画像（S01〜S{total_scenes}）を
Read ツールでチャットに表示し、ユーザーに確認してもらう。ファイル保存場所は変更しない
（Google Drive上に生成されたままでよい）— あくまで動画生成に進む前の確認ゲートを追加する。

```
{各シーン画像をReadツールで貼り付け}

上記でよろしいですか？（はい / 差し替えたいシーンを指定）
```

差し替え希望があれば該当シーンの `image_prompt` を修正して `sc_image_gen.py --scenes N` で
再生成し、再度そのシーンを表示して確認を取る。OKが出るまで STEP 5D には進まない。
