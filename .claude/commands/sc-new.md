...
- **忍者・間諜テーマが極めて強い（2026-08-04発見、2026-08-29のFable5監査で再検証・数値訂正）。**
  該当7本（ep020, 027, 035, 050, 063, 064, 065）は**7本ともCTR中央値超え**（二項検定
  p≈0.008）。**積極的に追加すべき。** ただし維持率は平均以下の傾向があるため、CTRで
  惹きつけた後に飽きさせない構成（謎解き・人物ドラマの軸を持たせる）を意識すること。
...
- **2026-08-29追加知見: 直近11本（ep069〜079）でCTRが有意に落ち込んだ（8月公開コホートの
  公開後14日CTRが6-7月比でp≈0.002）。原因はチャンネル全体の露出低下ではなく
  （本編インプレはむしろ8月に過去最高水準）、この期間に忍者・間諜/軍師系など
  実証済みの強テーマが1本も挟まれず、マイナー人物×key_event/character角度が
  連続したことが主因と推定される（Fable5分析）。** → **弱いトピック（未実証の
  マイナー人物・key_event/death/character角度）を3本以上連続させない。2〜3本に1本は
  忍者・間諜または軍師・参謀など実証済みテーマを挟む。**
...
⚠️ グレーゾーン・Opus判定（{N}件）:
  🔧 要修正→対応済み（{N}件）: S07「Japan's most powerful」等の重複表現を修正
