...
## STEP 5c — QA チェック（公開前品質確認）

STEP 5b 完了後、動画生成の**前**に QA を実施する。
画像と音声はすでに生成済みなので、ここで問題を修正しても動画コストは発生しない。

### テキスト QA（必須）

以下の観点で `scenes[*].narration`・`teaser_narration`・`shorts_narration`・`youtube_description` を全件レビューする：

**1-A. 歴史的事実チェック**
- 年号・出来事・人物関係に明らかな誤りがないか
- 因果関係・時系列の矛盾がないか

**1-B. 英語用語・ニュアンスチェック（必ず確認）**
- `dynasty` → 天皇家以外への誤用（豊臣・徳川などは **clan / regime / house** が適切）
- `shogun` / `emperor` / `lord` の混同
- 最上級の過度な多用（greatest / most powerful の連発）
...
**1-D. 画像プロンプト QA**
- 同一または類似構図の重複がないか（特に冒頭と終盤で同キャラが被っていないか）
- 実在武将の象徴（六文銭・葵紋・鹿角兜 等）を image_prompt に明示しているか
- プロンプト内に文字・テキストを描かせる指示が含まれていないか（AI生成は文字が崩れる）

### 画像 × ナレーション 整合性チェック（画像あり時）

Google Drive の `images/S01.png`〜`S{N}.png` が存在する場合、各シーン画像を Read ツールで読み込んでナレーションと照合する。
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  QA REPORT — ep{NNN}「{youtube_title}」
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

【テキスト QA】
  [CRITICAL] S{N}: {問題の説明} → 修正案: {修正案}
  [WARNING]  S{N}: {問題の説明} → 修正案: {修正案}
  ✅ 文法・スペル: 問題なし
...
【画像 × ナレーション 整合性】
  [WARNING] S{N}: 画像に {内容} が描かれているがナレーションは {内容} を示している
  ✅ 主要シーン: 問題なし
...
```
CRITICAL {N}件・WARNING {N}件が見つかりました。

[A] CRITICAL のみ修正して動画生成へ進む
[B] CRITICAL + WARNING をすべて修正して動画生成へ進む
[C] 問題なし / 修正不要 → そのまま動画生成へ進む
```

**修正を行う場合:**
- `episodes/ep{NNN}.json` の該当シーンの `narration` または `image_prompt` を直接編集
- 修正後は変更箇所を箇条書きで報告し、STEP 6 へ進む

> ⚠️ narration を修正した場合は TTS（音声）の再生成が必要。該当シーンを報告してユーザーに確認を取る。
