...
- **忍者・間諜テーマは有望だが、「極めて強い・積極的に追加すべき」は過大評価だったことが
  2026-09-06のFable監査で判明した。** 該当7本（ep020, 027, 035, 050, 063, 064, 065）の累計CTRは
  7/7が中央値超えだが、(a) ep065はimpr964で自チャンネルの信頼性閾値（impr1500未満は測定不能扱い）
  を満たさず、除外すると6/6、(b) 同月内CTR中央値との比較ではep035は同値で6/7、(c) ep063〜065は
  5日間で連続公開されており独立試行ではない（二項検定の前提に反する）、(d) 約10のテーマ仮説を
  走査した中から最良の結果を報告している（多重比較）。さらに維持率は7本中5本
  （ep020・027・064・065、および全体最下位のep065）が中央値未満で、CTRと維持率が両立していない。
  **「積極的に追加すべき」ではなく「2〜3本に1本の配分に留める、有望だが実証済みとまでは
  言えないテーマ」として扱うこと。** CTRで惹きつけた後に飽きさせない構成（謎解き・人物ドラマの
  軸を持たせる）は引き続き意識すること。

- **タイトル型別CTR/維持率の分析は、分類ルールが揺れると結論が変わる不安定な指標
  （2026-08-29のFable5監査で判明）。** 現行 `youtube_title` に機械的な正規表現分類を
  当てると型A/Dの該当数・平均CTRが手動分類時と大きくズレる（n・CTRとも別の値になる）。
  **型別のCTR・維持率数値そのものを意思決定の根拠にしない。** 下記「YouTubeタイトルの型」
  節の「主人公1人・1つの選択」への絞り込み推奨は構成上の指針として引き続き有効。

- **2026-08-29追加知見（2026-09-06のFable監査で方法論的な脆弱性を確認・トーンダウン）:**
  直近11本（ep069〜079）は公開後14日CTRの生データ上は6-7月コホートより低かった（p≈0.002）が、
  この分析には複数の脆弱性がある — (a) ep074〜079は14日測定窓が完了する前のデータで
  算出されている、(b) 11本中impr1500（自チャンネルの信頼性閾値）に達しているのはep078の
  1本のみで大半がクリック数一桁台、(c)「8月コホート」の境界（ep069始まり）は事後的に
  選ばれており、完全な14日窓を持つ8月公開分だけで見ると6-7月より低くない。
  **「主因はトピック選定」という因果的な結論は支持されない。** ただし新作へのインプレッション
  配分自体は6月と比べ明確に減少しており、該当期間にトピックの偏り（マイナー人物×
  key_event/character角度の連続）があったのも事実のため、予防的な配慮として運用ルールは
  維持する（根拠の強さは当初の記述より弱いことを踏まえた上で）: **弱いトピック（未実証の
  マイナー人物・key_event/death/character角度）を3本以上連続させない。2〜3本に1本は
  忍者・間諜または軍師・参謀などのテーマを挟む。**
...
- 知名度による効果（マイナー人物が有利）は分類ルール次第で0.19〜0.88ptと変動し、頑健な
  ...
  有名人物は検索流入の受け皿にもなる。
- **「知られざる軍師・参謀」タイプは有望な仮説だが、まだ「実証済み」ではない
  （2026-08-29のFable5監査で明確化）。** 黒田官兵衛（ep030 CTR9.58%・ep052 CTR6.16%で
  CTR1位・2位を独占）はn=2でうち1本が全体最大の外れ値のため、単独では強い証拠にならない。
  一方で山本勘助（ep075、key_event角度）は同じ軍師アーキタイプでもCTR1.99%と低調だった
...
そのまま使うと誤読する（2026-07の視聴減は主にShorts変動の寄与）。本編の状況を見るときは
月次インプレッション（本編のみ集計）と公開後14日の年齢調整済み指標を優先すること。
...
### キュー健全性チェック（必須、2026-09-06追加・Fable監査対応）

⚠️ 2026-09-06のFable監査で、STEP 1に書かれた選定ルール（弱いトピック3本連続禁止・有名人物3〜4本混在・
Bakumatsu連投回避等）が**文章として存在するだけで、実際のキュー投入順序を検査する手順がなく、
守られないまま運用されていた**ことが判明した。候補を確定する前に、必ず以下を機械的に確認する。

pending先頭6件程度と、直近の `status: "published" | "in_production"` 数件を対象に：

- **弱いトピック連続チェック:** 直近公開3本＋pending先頭3本の中に、忍者・間諜または
  「知られざる軍師・参謀」系のテーマ（実証済み・仮説検証中いずれも可）が1本も含まれていない場合は警告する
- **同一angle連続チェック:** pending先頭から3件以上で同じangleが連続していないか確認する
  （特に `death` / `key_event` / `character` の連続に注意）
- **同一era連続チェック:** pending先頭から3件以上で同じeraが連続していないか確認する
  （Bakumatsu等データの乏しいeraは特に厳しく見る）
- **有名人物比率チェック:** pending全体（未着手分）に信長・家康・秀吉・武蔵・信玄・謙信・西郷等の
  有名人物が最低1〜2件含まれているか確認する

問題が見つかった場合は候補確定前に表示し、並び替え（pending配列内での順序変更のみ。episode_idは
変更しない）の可否をユーザーに確認する：

```
⚠️ キュー健全性チェックで問題を検出しました：
- pending先頭6件が全て「未実証マイナー人物×death/character角度」で、忍者・軍師系のテーマが
  含まれていません
- Bakumatsu系が直近4件で3件を占めています

並び替えますか？（はい / このまま進める）
```

問題がなければ「✅ キュー健全性チェック: 問題なし」とだけ表示し、そのまま人物重複チェックへ進む。

### 人物重複チェック（必須、2026-09-06改訂・Fable監査対応）

⚠️ 2026-09-06のFable監査で、`person`フィールドの**文字列一致**だけに頼るチェックでは
表記ゆれ（例: ep023/ep044の`"mitsuhide"`とep099の`"akechi_mitsuhide"`）を検出できず、
実質的に同じ人物・同じ切り口のエピソードがすり抜けて追加された事例（ep099。ep023の
「裏切りの動機」・ep044の「本能寺後の13日間」とほぼ同内容を新角度と称していた）が判明した。
以下の2段階でチェックする。

#### ① 直近5件の再登場チェック（変奏候補の抑制。従来通り）

候補を提案する前に、直近5件の `status: "published" | "in_production"` のエピソードと
`person` フィールドを照合する。**表記ゆれの可能性がある場合は`characters/`フォルダの
既存ファイル名・過去エピソードの`title`を突き合わせ、同一人物かどうかを文字列一致に頼らず判断する。**

- 直近5件に同じ人物が含まれている場合 → その候補をスキップし、人物が重ならない最初の `pending` を選ぶ
- 同じ人物しか残っていない場合 → 候補を提案しつつ「直近に同じ人物が続いています」と警告する

#### ② 全履歴の内容重複チェック（新規・必須）

①とは独立に、候補の人物が**過去に一度でも登場している場合**（直近5件に限らない）、
`topics_queue.json`全体からその人物の全エントリ（`person`表記が異なっていても同一人物なら含める）の
`title`・`notes`を抽出し、候補の`notes`に書かれた差別化内容が既存エントリで既にカバーされている
範囲と実質的に重複していないかを確認する。重複が疑われる場合は、候補をそのまま確定せず
「具体的に何と重複するか」を示した上でユーザーに提示し、新角度が明確でない限り候補から外す
（L86-87の再登場ルールと同じ基準：「視聴者が前回と全く違う何かを得られるか」）。
新キャラクターとして`characters/{name}.txt`を新規作成する場合、既存の`characters/`に類似名の
ファイル（別表記の同一人物）がないか必ず確認する。
...
しかし当時の週5本ペースでOpusサブエージェントを2回/話（STEP 2A生成＋STEP 3Aチェック）呼ぶと
...
**この変更後、ep068のような史実知識ミスの再発は確認されていない。**

**2026-08-30〜: 制作リソースの都合で公開ペースを週5本→週3本に変更。** Opus呼び出し
頻度は週10回→週6回に下がり、上記の「利用上限圧迫」という制約は緩和されたが、
現状のSonnet生成+Opusチェック構成で実績上の問題が出ていないため、当面この構成は
維持する（ペースが変わったこと自体を理由に構成変更はしない）。
...
**YouTube タイトルの型（CTR最大化、2026-09-06改訂・Fable監査対応）:**

⚠️ 2026-09-06のFable監査で、本節が「必ずいずれかの型に当てはめること」と強制する一方、
上記の統計記述では「型別のCTR・維持率数値そのものを意思決定の根拠にしない」としており、
同一ファイル内で矛盾していたことが判明した。**型への機械的な当てはめは目的化させず、
型は例示に留める。** 必須なのは以下3要件のうち**2つ以上**を満たすことである
（2026-08-29のリタイトル改修方針・実例に基づく。project memory参照）：

1. **具体的な数字**（人数・日数・回数等。例: "200 Years", "2,000 Men. 10 Days."）
2. **逆説・矛盾**（一見矛盾する要素の並置。例: 「裏切り者なのに200年統治した」）
3. **主人公1人・1つの選択への絞り込み**（下記「登場人物数」参照。型選びより優先度が高い、
   最も頑健なシグナル）

型の例（参考。当てはめ自体を目的にしない）:
...
実例（2026-08-29リタイトル、改修後）:
- "2,000 Men. 10 Days. One Reason: To Buy His Best Friend Time to Win Japan."（ep078）
- "The Traitor Who Ruled Japan for 200 Years"（ep077）
- "The Farmer Who Became Kyoto's Most Feared Swordsman — And Was Executed as a Traitor"（ep076）

**実績データに基づく優先度:** 型A/Dの構文（"The X Who/That〜"）は維持率で優位という
複数回の分析結果があるため、同じ具体性を持たせられるなら型A/D寄りの構文を優先してよい。
ただし**CTRについて型別の優位・劣位は主張しない**（2026-08-04に訂正済み。2026-08-29の
監査では機械的な分類ルール自体が不安定であることも判明しており、型別のCTR・維持率数値
そのものを意思決定の根拠にしない）。型Bは「なぜ」の答えが動画内に明確にある場合に使う。

× 避ける: `"The History of Miyamoto Musashi"`（説明的すぎる、数字も逆説もない）
× 避ける: 抽象的な形容詞の羅列（"The Greatest / Most Powerful..." 等、数字・逆説を伴わないもの）
○ 良い: 上記1〜3の要件を満たす「人間ドラマ」としてフックを作る
...
**⚠️ 標準から外れる容姿的特徴は、テキスト指示だけに頼らず「参照画像」方式を使う
（2026-09-10改訂・ep099森蘭丸で判明・実証済み）:**

`characters/{name}.txt` への記載や、image_prompt本文への直接埋め込み（後述）だけでは、
`character_ref`経由の自動注入がシーン生成時に確実に反映されるとは限らない（若衆髷・総髪
などプロジェクト標準〈月代ありのちょんまげ〉から外れる特徴で頻発）。原因は
`sc_image_gen.py`のBASE_CONTEXTに「男性キャラは月代付き髷がデフォルト」という指示が
全生成で強制適用されており、個別シーンのテキスト指示がどれだけ強くてもこれと競合し、
シーンによって（特にキャラクターが2人並ぶ構図で、標準的な月代の人物が近くにいる場合）
標準の月代に巻き戻ることがある。ep099では最大10回以上のプロンプト再調整を経てもテキストのみ
では解決せず、実際に生成された画像そのものをGeminiに参照画像として渡す方式に切り替えて
初めて安定した。

**推奨手順（`sc_image_gen.py --ref-scene`、2026-09-10〜対応）:**
1. 対象人物が登場する1シーンをまず`--scenes N`で生成する
2. Claudeが目視で確認する（実在人物ならWebSearchで肖像・写真等の一般的イメージとも照合する）
3. 問題なければ、そのシーンIDを`--ref-scene N`に指定して残りの対象シーンを一括生成する
   （同じ`character_ref`を持つ全シーンにその画像が外見参照として自動的に渡される）
   ```bash
   python3 sc_image_gen.py --episode ep{NNN} --scenes 3
   # ↓ 目視確認後
   python3 sc_image_gen.py --episode ep{NNN} --ref-scene 3 --scenes 4,5,7,9,...
   ```
4. 全シーン生成後、Claudeが全シーンを横並びで再度目視確認し、容姿・髪型が統一されているか
   最終チェックする（QAツールの自動判定は「剃り込みの有無」等の粗い基準しか見ないため、
   これだけでは統一性を保証できない。必ず目視で確認すること）
5. それでも特定シーンだけ崩れる場合（ep099のS11: 2人の構図で近接比較により干渉した事例）は、
   参照画像を対象人物の頭部のみ切り出したクロップ画像に絞り込むと改善することがある

image_prompt本文への直接埋め込み（下記）は、この参照画像方式と併用する補助的な手段として
引き続き有効だが、**単独では不十分**なことをふまえ、標準から外れる容姿がある場合は必ず
参照画像方式を主軸にすること。
...
2. 全ナレーションのファクトチェック・英語QA・画像プロンプトQA・視聴者獲得QAを行う
...
4. **⚠️（グレーゾーン・解釈上の論点）についても、この同じパスの中でひとつずつ
   「修正すべきか、許容範囲としてこのまま進めてよいか」を判定する（2026-08-31〜）。**
   - 判定基準: 史実として明確に不正確／誤解を招く／学説が割れているのに断定調で
     語っている、といった実害があるものは「要修正」。解釈の幅・通説的な言い回し・
     軽微な表現の好みの範囲は「許容」。
   - 「要修正」と判定したものは、❌と同様にユーザー確認なしにその場で
     `episodes/ep{NNN}.json` の該当シーンを直接編集して修正する
   - 「許容」と判定したものは修正せず、判定理由とともに制作確認書に記録する
   - 保存される制作確認書の【⚠️ 注意・議論あり】には、各項目に判定結果
     （許容 / 要修正→対応済み）と理由を併記する。すべて判定済みの状態で保存し、
     未判定のまま⚠️を残してはならない

   **⚠️自動修正の適用範囲外（2026-09-06追加・Fable監査対応）:** `youtube_title` /
   S01（hook）のナレーション / `teaser_narration` / `shorts_narration` /
   `shorts_hook_lines` / `thumbnail_prompt` / `shorts_face_image_prompt` は、
   史実の❌・⚠️が見つかった場合でも**このステップでの自動修正の対象外**とする。
   これらは視聴者獲得に直結する訴求要素であり、史実精度の観点だけで機械的に
   書き換えると訴求力を弱める方向にしか働きえないため。該当箇所に問題が見つかった場合は
   修正を実行せず、修正案を制作確認書の【⚠️ 注意・議論あり】に「要検討（自動修正対象外）」
   として記録し、統合確認でユーザーの判断を仰ぐ。
5. 以下のフォーマットで `ep{NNN}_制作確認書.txt` を組み立てて `~/Desktop/SC/` に保存する
...
================================================================
【視聴者獲得QA】（2026-09-06追加・Fable監査対応）
================================================================

⚠️ 2026-09-06のFable監査で、制作確認書に史実精度・英語QA・画像プロンプトQAの項目はあっても
「このエピソードが視聴者を惹きつけ、離脱させない構成になっているか」を検証する項目が
一つもなかったことが判明した。以下をチェックし、問題があれば記載する
（問題なければ各項目「✅ 問題なし」）。

**このQAは史実精度チェックとは異なり、上記❌・⚠️の自動修正の対象には含めない。**
タイトル・Hook・Shorts訴求要素は視聴者獲得の核であり、機械的な自動書き換えは
訴求を弱める方向にしか働きうるリスクがあるため、問題が見つかった場合は修正案を提示するに
留め、制作確認書に記録した上でユーザー確認（統合確認）に委ねる。

【タイトル・Hook】
- `youtube_title` が「具体的な数字」「逆説・矛盾」「主人公1人・1つの選択」のうち
  2つ以上を満たしているか
- タイトルで示唆された主張（例:「〜を変えた」「たった13日で」）がナレーション内で
  実際に回収されているか（空約束になっていないか。典拠不明の具体的な期限・数字表現に注意）
- hook（S01）の1文目が10語以内で、数字または問いかけを含んでいるか

【サムネイル・Shorts】
- `thumbnail_prompt` に、人物の表情・肩書き的テキストオーバーレイの指定が含まれているか
- `shorts_hook_lines` が3行固定・各行の文字数上限（14/18/26文字）を満たしているか
- `shorts_face_image_prompt` が視線・表情のドラマ性を明確に指定しているか

【総評】
視聴者獲得の観点で懸念があれば1〜2文でまとめる。問題なければ「訴求力に懸念なし」と記載する。
...
**ナレーション⚠️は STEP 3A のOpusパス内で判定・（必要なら）修正済みのため、
STEP 5A/5B前の早期確認ゲートは不要（2026-08-31〜）。** 以前は⚠️が残っている場合のみ
ユーザー承認を待ってからTTS/画像生成に進んでいたが、Opusが「修正すべきか許容範囲か」を
その場で判定し必要な修正は即座に反映する運用に変更したため、この報告は常に情報提供のみで、
確認を待たずそのまま STEP 5A/5B へ進む。

**画像プロンプトQAの⚠️は確認なしに即座に自動修正する（2026-08-04〜）。** 理由: 画像プロンプトの
修正はTTS/画像生成の「前」に行う変更であり、間違っていても後段のSTEP 5C（画像QA・自動2回リトライ
＋Claude一次判断）でもう一段検出される安全網がある。機械的・構造的な指摘（写実指定句の欠落、
他カットとの構図重複など）が大半のため、ユーザー確認を挟む必要性が薄い。
`episodes/ep{NNN}.json` の該当箇所を直接編集し、「対応済み」として報告する（「対応してよいか」とは聞かない）。

```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📋 ファクトチェック・QA 結果
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
❌自動修正済み（{N}件・修正前後を1件ずつ列挙する）:
  🔧 S05: "1582年" → "1600年" に修正（ep.json 更新済み）
  🔧 S03: "Toyotomi dynasty" → "Toyotomi clan" に修正済み

⚠️ グレーゾーン・Opus判定（{N}件・要修正→対応済みは修正前後を1件ずつ列挙する）:
  🔧 S07: "Japan's most powerful" → "one of Japan's most powerful" に修正済み
  ✅ 許容・修正不要（{N}件）: S04「金蔵掌握の描写」等（判定理由は制作確認書参照）

🖼️ 画像プロンプトQA・自動対応済み（{N}件）:
  🔧 S02/S05/S06...: 写実指定句を追記
  🔧 S01: サムネイルとの構図重複を解消

🎯 視聴者獲得QA（自動修正なし。対象は`youtube_title`/S01hook/`teaser_narration`/
`shorts_narration`/`shorts_hook_lines`/`thumbnail_prompt`/`shorts_face_image_prompt`）:
  ✅ 問題なし、または
  💡 要検討（{N}件・修正案のみ提示、ep.jsonは変更していない）: {項目}: {懸念点} → 修正案: "{案}"
...
この報告は常に情報提供のみ。ユーザーの確認を待たず、そのまま STEP 5A/5B へ進む
（ナレーション⚠️・画像プロンプトQA⚠️とも、この時点までにOpus・オーケストレーターの
判定と必要な修正が完了しているため）。サムネイル・BGMの見直しはここでは扱わない
（STEP5C後の統合確認で扱う）。**視聴者獲得QAで「要検討」が出た項目（自動修正対象外）も
ここでは修正せずSTEP 5A/5B以降へ進み、STEP5C後の統合確認でまとめてユーザーに提示する**
（TTS/画像生成の前提にはならないため、確認を遅らせても後続は無駄にならない）。
...
**⚠️ ナレーション音声はHTMLにbase64で直接埋め込まれるため、生成後に音声を再生成しても
自動反映されない（2026-08-29修正時点の設計。画像・BGMは同じディレクトリツリー内で完結する
ため相対パス参照になっており、こちらはファイルを差し替えればブラウザ再読み込みだけで
最新化される）。統合確認・確認修正ループの中でナレーション音声を1本でも再生成した場合は、
ユーザーに再確認を求める前に必ず `sc_scene_review.py` を再実行してページを最新化すること
（画像のみの再生成であれば再実行不要）。**
...
Gemini Visionへの委任に変更した（使用モデルは`sc_zoom_anchor.py`の`MODEL`定数を参照。
Geminiのモデル更新に応じて随時変わるためここには固定で書かない）。Gemini側の追加コストは
flashティアの単純なVisionタスクのため無視できる水準。

```bash
python3 sc_zoom_anchor.py --episode ep{NNN}
```

対象シーン（`character_ref`が設定されているシーン全て）について、各シーン画像を
Geminiに渡し、まず主要な人物が1人（SINGLE）か2人が共同主体の構図（TWO_SHOT）かを
判定させる（**2026-08-26〜: image_promptの"on the left"/"on the right"キーワード判定は
廃止**。"facing"等のキーワードだけで両者を伴わない二人シーンを1人構図と誤判定していた
問題がep088で発覚したため、判定自体をGemini Visionに委ねる方式に変更した）。
SINGLEなら主被写体の「顔〜胸」あたりの重心を正規化座標（x: 0.0=左端〜1.0=右端、
y: 0.0=上端〜1.0=下端）で、TWO_SHOTなら`{"multi_person": true}`を
`episodes/ep{NNN}.json` の該当シーンに直接書き込む（スクリプト内で完結）。
`character_ref`が無いシーンはスキップされ、zoom_anchor は null のまま。
`sc_video_gen.py`はmulti_personの場合、1点ズームではなく両者を収める
pan_zoom_outに切り替える。

**⚠️ 判定精度に不安がある場合:** `sc_video_gen.py`で生成した動画のズーム構図
（顔が変に切れていないか等）を確認し、目立ったズレがあれば `--scenes N` で
該当シーンのみ再判定するか、`episodes/ep{NNN}.json` の `zoom_anchor` を手動で
微調整してよい。
