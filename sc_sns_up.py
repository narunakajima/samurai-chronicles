...
def find_next_publish_slot() -> str:
    ...
    配信曜日: 火・木・土（2026-08-30〜、制作リソース不足のため週5本→週3本に変更）
    - episodes/*.json の scheduled_at を読んで使用済み日付を収集
    - 今日の 03:00 JST が未来 → 今日を候補に、過去 → 明日を候補に
    - 配信曜日でない日 および 使用済み日付を避けて最初の空き日を返す

    03:00 JSTは米国側では前日昼頃（東部14時/太平洋11時前後、DSTにより変動）に
    あたり、米国の平日・日中に公開して夜のプライムタイム視聴に間に合わせる設計。
    火木土（JST）は米国側で見ると月水金にあたり、5日から均等に間引いた形。
    """
    # 配信可能な曜日: 火(1), 木(3), 土(5)。米国側では月・水・金にあたる
    PUBLISH_WEEKDAYS = {1, 3, 5}  # 0=月, 6=日 はスキップ
