...
- `sc_bgm_final_check.py` の判定結果（各曲評価＋総合GO/差し替え推奨）を、
  ユーザーへの提示メッセージに含める。
- **選定3曲がそのまま残る `~/Desktop/SC/BGM/` を確認場所として案内し、
  ユーザー自身に試聴してもらった上でOKを得る**（2026-08-04〜: この時点で `BGM/` 内は
  既に不選択の6曲を削除済みで選定3曲のみになっているため、`~/Desktop/SC/` 直下への
  コピーは行わない。2026-08-01〜08-04まではコピーしていたが、「9曲候補の中から3曲を
  探しづらい」という当初の問題は削除処理で解消済みのため、同一ファイルを二重に置くだけの
  無駄な手順だとユーザーから指摘された）。SendUserFileはmp3をインラインプレビューできない
  ため使わない。`open` コマンドでFinderを自動的に開く処理も行わない（2026-08-04〜廃止）。
  ここは自動確定せず、明示的な承認を待って初めて次に進む。
- OKが出るまで STEP 4（BGM紐付け・Google Driveへの移動）には進まない。
- 差し替え希望があれば該当役割のみ再選定（別候補への切替、または新規クエリで再検索）し、
  再度 `sc_bgm_final_check.py` → 試聴確認の順で確認を求める。
...
❌自動修正済み・🖼️画像プロンプト対応済み・⚠️なし → 「✅ QA完了 — 問題なし」を表示。
ナレーション面はここで確定するが、STEP 4 へは**サムネイル・BGMのユーザー確認が取れてから**
進む（自動では進まない）。⚠️（ナレーション）がある場合は内容を表示し、ナレーション修正確認と
サムネイル・BGM確認をまとめてユーザーに求めてよい。
